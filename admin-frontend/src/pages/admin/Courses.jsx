...
export default function Courses() {
  ...
  // Fetch courses (with array safety)
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/admin/courses/${deleteTargetId}`);
      setCourses(courses.filter((c) => c.id !== deleteTargetId));
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <h2 style={{ color: '#0A2956', fontWeight: 700 }}>Courses</h2>
        <button
...
function DetailRow({ label, value, mono }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
      ...
      <span style={{ color: '#0A2956', fontWeight: 400, textAlign: 'right', wordBreak: 'break-all', fontFamily: mono ? 'monospace' : 'inherit' }}>{value}</span>
    </div>
  );
}
