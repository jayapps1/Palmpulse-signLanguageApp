...
export default function Lessons() {
  ...
  const [formVideoFile, setFormVideoFile] = useState(null); // actual File object
  const [formImageFile, setFormImageFile] = useState(null);

  // ── Media viewer for Manage Signs modal ──
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [lessonsRes, coursesRes, usersRes] = await Promise.all([
  ...
  const fetchAllSigns = async () => {
    ...
    try {
      const { data } = await api.get('/admin/signs');
  ...
  const handleAddSign = async (signId) => {
    try {
      ...
      setToast({ message: 'Failed to add sign', type: 'error' });
    }
  };
  ...
  const handleRemoveSign = async (signId) => {
    try {
      ...
      setToast({ message: 'Failed to remove sign', type: 'error' });
    }
  };
  ...
  // ── Create or Update handler (now with optional sign creation) ──
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ── If a video file was selected, upload it and create a sign linked to this lesson ──
      if (formVideoFile || formImageFile) {
        ...
        if (formVideoFile) {
          ...
          try {
            ...
            const vRes = await api.post('/signs/upload', vForm, {
              headers: { 'Content-Type': 'multipart/form-data' },
            });
            videoUrl = vRes.data.url;
          } catch (err) {
            setToast({ message: 'Video upload failed, but lesson was saved', type: 'error' });
          } finally {
        ...
        if (formImageFile) {
          ...
          try {
            ...
            const iRes = await api.post('/signs/upload', iForm, {
              headers: { 'Content-Type': 'multipart/form-data' },
            });
            imageUrl = iRes.data.url;
          } catch (err) {
            setToast({ message: 'Image upload failed, but lesson was saved', type: 'error' });
          } finally {
            setUploadingImage(false);
          }
        }

        // Create sign if we have at least one media URL
        if (videoUrl || imageUrl) {
          const signName = form.title || 'Untitled Sign';
          try {
            const signPayload = {
              signName,
            ...
            const signRes = await api.post('/admin/signs', signPayload);
            // Link sign to lesson
            await api.post(`/lessons/${lessonId}/signs/${signRes.data.signId}`);
            setToast({ message: 'Sign created and linked!', type: 'success' });
          } catch (err) {
            setToast({ message: 'Sign creation failed, but lesson was saved', type: 'error' });
          }
        }
      }
  ...
  const myCourses = useMemo(() => {
    return courses.filter((c) => c.createdBy === currentUserId);
  }, [courses, currentUserId]);

  const getCourseCreatorName = (createdBy) => {
    if (!createdBy && createdBy !== 0) return 'Unknown';
    if (createdBy === currentUserId) return 'You';
    return userMap[createdBy] || `User #${createdBy}`;
  };
  ...
  const truncateCourseTitle = (courseId) => {
    const title = getCourseTitle(courseId);
    return truncate(title, 7);
  };
  ...
  const getCreatorNameForForm = (lesson) => {
    if (!lesson) return currentUserFullName;
    const name = userMap[lesson.createdBy] || `User #${lesson.createdBy}`;
    return lesson.createdBy === currentUserId ? 'You' : name;
  };

  // ── Media viewer for sign links ──
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <h2 style={{ color: '#0A2956', fontWeight: 700 }}>Lessons</h2>
        <button
      ...
      {showForm && (
        ...
        >
          ...
          <select name="courseId" value={form.courseId} onChange={handleChange} style={inputStyle}>
            ...
            {myCourses.map((c) => (
              <option key={c.id} value={c.id}>
                {c.title} — {getCourseCreatorName(c.createdBy)}
              </option>
            ))}
          </select>

          <div style={{ gridColumn: 'span 2', fontSize: '0.9rem', color: '#64748B' }}>
            Created by: <strong>{getCreatorNameForForm(editingLesson)}</strong>
          </div>
          ...
          {/* ── Video upload for lesson (optional) ── */}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filteredLessons.map((l) => (
                <tr key={l.lessonId} style={{ borderBottom: '1px solid #E2E8F0' }}>
                  ...
                  <td style={tdStyle} title={l.courseId ? getCourseTitle(l.courseId) : ''}>
                    {l.courseId ? truncateCourseTitle(l.courseId) : '—'}
                  </td>
                  <td style={tdStyle}>
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                      ...
                      <button onClick={() => setManagingQuizFor(l.lessonId)} style={iconButtonStyle} title="Manage Quiz">📝</button>
                    </div>
                  </td>
                </tr>
              ))}
              {filteredLessons.length === 0 && (
                <tr>
                  <td colSpan="6" style={{ textAlign: 'center', padding: '2rem', color: '#64748B' }}>No lessons found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(10, 41, 86, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '1rem' }} onClick={() => setSelectedLesson(null)}>
          <div style={{ background: '#FFFFFF', borderRadius: '16px', maxWidth: '550px', width: '100%', padding: '2rem', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', maxHeight: '80vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <DetailRow label="Created by" value={getCreatorDisplay(selectedLesson.createdBy)} />
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', background: toast.type === 'success' ? '#0A2956' : '#EF4444', color: 'white', padding: '0.85rem 1.5rem', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', zIndex: 3000, fontWeight: 500, fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {managingQuizFor && (
            <QuizManager
              lessonId={managingQuizFor}
              userRole={currentUser.role}
              onClose={(refresh) => {
                setManagingQuizFor(null);
