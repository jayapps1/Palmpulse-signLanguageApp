...
export default function TeacherSigns() {
  ...
  const [playingMediaType, setPlayingMediaType] = useState('video');
  ...
  const filteredSigns = useMemo(() => {
    ...
    return signs.filter(s => s.signName?.toLowerCase().includes(term) || s.description?.toLowerCase().includes(term));
  }, [signs, searchTerm]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const payload = { signName: form.signName, description: form.description, videoUrl: form.videoUrl, imageUrl: form.imageUrl };
    ...
    try {
      if (editingSign) {
        ...
        await api.post('/teacher/signs', payload);
  ...
  const resetForm = () => { setForm(DEFAULT_FORM); setEditingSign(null); setShowForm(false); setVideoPreviewUrl(null); };
  const openEdit = (sign) => { setEditingSign(sign); setForm({ signName: sign.signName || '', description: sign.description || '', videoUrl: sign.videoUrl || '', imageUrl: sign.imageUrl || '' }); setShowForm(true); };
  const openCreate = () => { setEditingSign(null); setForm(DEFAULT_FORM); setShowForm(true); };
  const handleChange = (e) => { setForm({ ...form, [e.target.name]: e.target.value }); };
  const getMediaUrl = (path) => { if (!path) return null; return path.startsWith('http') ? path : `http://localhost:8085${path}`; };
  ...
  const handleVideoUpload = async (file) => {
    if (!file) return; setUploadingVideo(true);
    try {
      const formData = new FormData(); formData.append('file', file); formData.append('type', 'video');
      const { data } = await api.post('/signs/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      setForm(prev => ({ ...prev, videoUrl: data.url }));
      setToast({ message: 'Video uploaded!', type: 'success' });
    } catch (err) { setToast({ message: 'Video upload failed', type: 'error' }); } finally { setUploadingVideo(false); }
  };
  ...
  const captureThumbnail = async () => {
    ...
    canvas.toBlob(async (blob) => {
      if (!blob) return; setUploadingImage(true);
      try {
        const formData = new FormData(); formData.append('file', blob, 'thumbnail.jpg'); formData.append('type', 'image');
        const { data } = await api.post('/signs/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
        setForm(prev => ({ ...prev, imageUrl: data.url }));
        setToast({ message: 'Thumbnail captured!', type: 'success' });
      } catch (err) { setToast({ message: 'Thumbnail upload failed', type: 'error' }); } finally { setUploadingImage(false); }
    }, 'image/jpeg', 0.9);
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    } catch (err) { setToast({ message: 'Image upload failed', type: 'error' }); } finally { setUploadingImage(false); }
  };

  const truncate = (text, maxLen = 7) => { if (!text || typeof text !== 'string') return ''; return text.length > maxLen ? text.substring(0, maxLen) + '…' : text; };

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <h2 style={{ color: '#0A2956', fontWeight: 700 }}>My Signs</h2>
        <button onClick={() => showForm ? resetForm() : openCreate()} style={{ padding: '0.6rem 1.5rem', background: '#0A2956', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem' }}>
          {showForm ? 'Close Form' : '+ Add Sign'}
        </button>
      </div>

      {error && <div style={{ background: '#FFF0F0', color: '#B00020', padding: '0.75rem 1rem', borderRadius: 8, marginBottom: '1rem', border: '1px solid #FFCDD2' }}>{error}</div>}

      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '1.5rem', marginBottom: '2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          <div style={{ gridColumn: 'span 2' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
              <button type="button" onClick={() => videoInputRef.current.click()} disabled={uploadingVideo} style={uploadButtonStyle}>{uploadingVideo ? 'Uploading...' : 'Choose Video'}</button>
              <input type="file" ref={videoInputRef} onChange={handleVideoFileSelect} accept="video/*" style={{ display: 'none' }} />
              {form.videoUrl && !uploadingVideo && <span style={{ color: '#135290', fontSize: '0.9rem' }}>✓ Uploaded</span>}
            </div>
            {videoPreviewUrl && (
              <div style={{ marginTop: '0.75rem', position: 'relative' }}>
                <video ref={videoPreviewRef} src={videoPreviewUrl} controls style={{ width: '100%', maxHeight: '200px', borderRadius: '8px', background: '#000' }} />
                <button type="button" onClick={captureThumbnail} disabled={uploadingImage} style={{ marginTop: '0.5rem', padding: '0.3rem 0.8rem', background: '#0A2956', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '0.8rem' }}>Capture Thumbnail</button>
              </div>
            )}
          </div>
          ...
          <div style={{ gridColumn: 'span 2' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
              <button type="button" onClick={() => imageInputRef.current.click()} disabled={uploadingImage} style={uploadButtonStyle}>{uploadingImage ? 'Uploading...' : 'Upload Image'}</button>
              <input type="file" ref={imageInputRef} onChange={handleImageUpload} accept="image/*" style={{ display: 'none' }} />
              {form.imageUrl && <span style={{ color: '#135290', fontSize: '0.9rem' }}>✓ Thumbnail ready</span>}
            </div>
            {form.imageUrl && <img src={getMediaUrl(form.imageUrl)} alt="Thumbnail" style={{ width: '100px', borderRadius: '8px', marginTop: '0.5rem' }} />}
          </div>
          ...
          <div style={{ gridColumn: 'span 2', display: 'flex', gap: '1rem' }}>
            ...
            <button type="button" onClick={resetForm} style={{ flex: 1, padding: '0.8rem', background: '#E2E8F0', color: '#0A2956', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer', fontSize: '1rem' }}>Cancel</button>
          </div>
        </form>
      )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filteredSigns.map((s) => (
                <tr key={s.signId} style={{ borderBottom: '1px solid #E2E8F0' }}>
                  ...
                  <td style={tdStyle}>
                    {s.videoUrl ? (
                      <button
                        onClick={() => { setPlayingSign(s); setPlayingMediaType('video'); }}
                  ...
                  <td style={tdStyle}>
                    {s.imageUrl ? (
                      <button
                        onClick={() => { setPlayingSign(s); setPlayingMediaType('image'); }}
                        style={{ background: 'none', border: 'none', color: '#135290', cursor: 'pointer', textDecoration: 'underline', fontSize: '0.9rem' }}
                      >
                        🖼 View
                      </button>
              ...
              {filteredSigns.length === 0 && (
                <tr>
                  <td colSpan="6" style={{ textAlign: 'center', padding: '2rem', color: '#64748B' }}>No signs found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(10, 41, 86, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '1rem' }} onClick={() => setSelectedSign(null)}>
          <div style={{ background: '#FFFFFF', borderRadius: '16px', maxWidth: '550px', width: '100%', padding: '2rem', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', maxHeight: '80vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <DetailRow label="Created by" value={selectedSign.createdBy} />
      ...
      {/* Media Viewer Modal */}
      {playingSign && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }} onClick={() => setPlayingSign(null)}>
          <div style={{ background: '#FFFFFF', borderRadius: '12px', padding: '1.5rem', maxWidth: '80%', width: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
              <h3 style={{ color: '#0A2956', margin: 0 }}>{playingSign.signName}</h3>
              <button onClick={() => setPlayingSign(null)} style={{ background: 'none', border: 'none', fontSize: '1.3rem', cursor: 'pointer', color: '#94A3B8' }}>✕</button>
            </div>
            {playingMediaType === 'video' && playingSign.videoUrl && (
              <video controls autoPlay src={getMediaUrl(playingSign.videoUrl)} style={{ width: '100%', borderRadius: '8px', maxHeight: '70vh' }} />
            )}
            {playingMediaType === 'image' && playingSign.imageUrl && (
              <img src={getMediaUrl(playingSign.imageUrl)} alt={playingSign.signName} style={{ width: '100%', borderRadius: '8px', maxHeight: '70vh' }} />
            )}
          </div>
        </div>
      )}
